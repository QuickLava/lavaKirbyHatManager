	...
	if (hatsIn.is_open())
	{
		...
		if (!toAdd.empty())
		{
			*kHCS << "\nAdding Hats...\n";
			...
			{
				*kHCS << "\n";
				lava::brawl::kirbyhat::summarizeHats(*kHCS, moduleFile, kbxFile);
			}
			...
			*kHCS << "\nAdding Hats...\n";
			...
			for (std::size_t i = 0; i < toAdd.size(); i++)
			{
				*kHCS << "EX Character ID: 0x" << lava::numToHexStringWithPadding(toAdd[i].first, 0x04) << ", Source Character ID: 0x" << lava::numToHexStringWithPadding(toAdd[i].second, 0x04) << "\n";
				...
				if (doModuleEdit)
				{
					lava::brawl::kirbyhat::neoAddHatToREL(relIn, toAdd[i].first, toAdd[i].second);
				}
			}
			if (relIn.fileBody.populated() && kbxFile.populated())
			{
				*kHCS << "\n";
				lava::brawl::kirbyhat::neoSummarizeHats(*kHCS, relIn, kbxFile);
			}
